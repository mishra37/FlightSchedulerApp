...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // initializes PriorityQueue object and shortest path object as null
    PriorityQueue<Path> pq = new PriorityQueue<>();
    Path shortestPath = null;
    // visited object of hashSet which stores the visited vertices
    Set<Vertex> visited = new HashSet<>();

    // throws an exception when no path can be found between start and end vertices
    if (!(vertices.containsKey(start) && vertices.containsKey(end)))
      throw new NoSuchElementException();

    // initialize the Path and distance
    Map<T, Integer> distance = init(start, pq);

    // process queue entries
    while (!pq.isEmpty()) {
      Path current = pq.remove();
      Vertex currentVertex = current.end;
      if (currentVertex.data.equals(end)) {
        shortestPath = setShortestPath(shortestPath, current);
      }
      // checks each unvisited vertex in the path
      if (!visited.contains(currentVertex)) {
        // add is as visited in the hashMap
        visited.add(currentVertex);
        // traverses through each edge from current Vertex
        for (Edge e : currentVertex.edgesLeaving) {
          // changes weight of successor vertex if its total initial weight is greater than
          // sum of current vertex weight plus edge weight
          if (distance.get(e.target.data) > distance.get(currentVertex.data) + e.weight) {
            distance.put(e.target.data, distance.get(currentVertex.data) + e.weight);
            Path pathNext = new Path(current);
            pathNext.extend(e);
            pq.add(pathNext);
          }
        }
  ...
  /**
   * a private method which sets initial distances of each vertex as MAX_VALUE and stores it in Map.
   * Also adds initial vertex to Path and sets its distance to 0
   * 
   * @param start data item within first node in path
   * @param pq    PriorityQueue object which stores path
   * @return Map of distance of all vertices
   */
  private Map<T, Integer> init(T start, PriorityQueue<Path> pq) {
    // set distance of each vertex to Integer MAX_VALUE
    Map<T, Integer> distance = new HashMap<>();
    for (T t : vertices.keySet()) {
      distance.put(t, Integer.MAX_VALUE);
    }
    // add initial vertex to Path and set distance to 0
    Vertex v = vertices.get(start);
    Path initialPath = new Path(v);
    pq.add(initialPath);
    distance.put(v.data, 0); // set initial distance to 0
    return distance;
  }

  /**
   * a private method which returns a shortestPath after comparing distance from current path to
   * that of shortest found path
   * 
   * @param shortestPath - shortest path to the target vertex
   * @param current      - current path to the target vertex
   * @return
   */
  private Path setShortestPath(Path shortestPath, Path current) {
    // if no shortest path is found then, it just sets current path
    // to be shortest
    if (shortestPath == null) {
      shortestPath = current;
    }
    // sets shortest path to be current path if distance from that path to target vertex
    // is less than shortest path found
    else {
      if (current.distance < shortestPath.distance) {
        shortestPath = current;
      }
    }
    return shortestPath;
